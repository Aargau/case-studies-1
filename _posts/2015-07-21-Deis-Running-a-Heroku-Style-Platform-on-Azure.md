...
excerpt: Deis: Running a Heroku-Style Platform on Azure
---

# Deis: Running a Heroku-Style Platform on Azure

Felix Rieseberg, April 20, 2015, 1 – Based on a draft by Patrick Chanzeon from February 24  

# Tags

Azure Virtual Machines, Linux, CoreOS, Azure Python SDK, Python, Docker, Devops, PaaS

This case study describes the collaboration between the open source platform Deis and the Strategic Engagements team, ultimately enabling customers to run Deis on Azure.
...
By default, the script will provision a three-node cluster, but you can increase the number of nodes with the num-nodes parameter. Note that for scheduling to work properly,</span> [clusters must consist of at least three nodes and always have an odd number of members](https://github.com/coreos/etcd/blob/master/Documentation/optimal-cluster-size.md).
